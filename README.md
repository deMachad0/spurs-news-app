# Hotspur Tottenham APP

This app shows Spurs news and fixtures using a football API (Football-Data.org).

## 💡 Learning Materials 

This directory contains all the foundational content, notes, and useful materials to build an application

## Step by step

This directory contains all the steps behind an application, APIs (Football-Data.org), Coding

## Tools

This directoy uses Python language for coding, Postman to test API calls, HTML for basic display, CSS to style and clean up appearance.


## How to run the app

"pip install requests", Install Python, install libraries, run "main.py"

## Libraries and Dependencies 

"requests" for API calls

### 🗓️ Key Deadlines

* Project will be updated weekly


## 👨‍💻 Author

* Andre


---
